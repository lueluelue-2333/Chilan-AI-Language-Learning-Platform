...
def main():
    ...
    parser.add_argument(
        "target",
        nargs="?",
        default=str(Path(__file__).resolve().parent / "synced_json"),
        help="JSON file or directory to process. Defaults to backend/content_builder/synced_json.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    parser.add_argument(
        "--global-memory",
        action="store_true",
        help="Backfill backend/content_builder/global_vocab_memory.json using synced_json lessons.",
    )
    ...
    if args.global_memory:
        ...
        lesson_dir = Path(__file__).resolve().parent / "synced_json"
