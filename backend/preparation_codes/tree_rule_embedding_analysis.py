...
def train_tree_rule_model(train_df, test_df):
    feature_names = list(MODELS.keys())
    X_train = train_df[feature_names].values
    y_train = train_df["label"].values
    X_test = test_df[feature_names].values
    ...
    print("🌲 正在训练规则树模型 (DecisionTreeClassifier)...")
...
def plot_tree_rules(model, eval_mode):
...
def run_tree_rule_analysis():
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        for eval_mode in ["English_Evaluation", "Chinese_Evaluation"]:
            ...
            logistic_model, logistic_metrics = train_logistic_baseline(train_df, test_df)
            tree_model, result_df, tree_metrics = train_tree_rule_model(train_df, test_df)
            ...
            print(f"  Confusion Matrix [ [TN, FP], [FN, TP] ]: {tree_metrics['confusion_matrix']}")
            ...
            result_df.to_excel(writer, sheet_name=f"{eval_mode[:3]}_tree_test", index=False)
            pd.DataFrame([{"rules": rule_text}]).to_excel(writer, sheet_name=f"{eval_mode[:3]}_rules", index=False)

            print(f"📈 [{eval_mode}] 正在绘制决策树规则图...")
            plot_tree_rules(tree_model, eval_mode)
